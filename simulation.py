...
from repo import contar_objetos_executando_no_equipamento,    proxima_pericia, objetos_aguardando_equipamento,    mover_objeto_para_buffer_do_proximo_equipamento, numero_de_vagas_no_equipamento
...
def marcar_como_finalizados_objetos_que_finalzaram_etapa(method: SIM_METHOD, time: TimeValue, db_session: Session, commit=True) -> None:
    ...
    if method == "current" and config.PLANTAO["current"]:
...
def puxar_do_buffer_para_execucao(equipment: Equipment, time: TimeValue, db_session: Session) -> None:
...
def update_lab(method: SIM_METHOD, time: TimeValue, db_session: Session) -> None:
    if not e_horario_de_trabalho(method, time.time):
        return
    marcar_como_finalizados_objetos_que_finalzaram_etapa(method, time, db_session)
    tirar_do_nome_do_perito_as_finalizadas(db_session)
    if method == "current":
        atribuir_novas_pericias_aos_peritos_ociosos(time, db_session)
    ...
    for equipment in query.all():
        ...
        objetos = objetos_aguardando_equipamento(equipment, time, n, db_session)
        for objeto in objetos:
            mover_objeto_para_buffer_do_proximo_equipamento(objeto, db_session)
        puxar_do_buffer_para_execucao(equipment, time, db_session)
...
def e_horario_de_trabalho(method: SIM_METHOD, time: datetime) -> bool:
    if config.PLANTAO[method]:
        return True 
